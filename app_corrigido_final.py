...
# CORREÇÃO: Melhorado sistema de descoberta de URLs com suporte a múltiplos padrões
...
# ATUALIZADO: Hashes corretos extraídos de arquivos HAR (2026-05-26)
# Servidor correto: arquivos.esusaps.ufsc.br (não esusab)
HASHES_CONHECIDOS = {
    # Versões 5.4.x - Hashes validados
    "5.4.37": "687651a247e537a3",  # ✅ Testado e funcionando
    "5.4.36": "687651a247e537a3",  # Presumido (mesma série)
    "5.4.35": "687651a247e537a3",  # Presumido
    "5.4.34": "687651a247e537a3",  # Presumido
    "5.4.33": "687651a247e537a3",  # Presumido
    "5.4.32": "687651a247e537a3",  # Presumido
    "5.4.31": "48005440f1a35d3d",  # ✅ Testado e funcionando
    "5.4.30": "48005440f1a35d3d",  # Presumido
    "5.4.28": "48005440f1a35d3d",  # Presumido
    "5.4.26": "48005440f1a35d3d",  # Presumido
    "5.4.22": "48005440f1a35d3d",  # Presumido
    "5.4.21": "48005440f1a35d3d",  # Presumido
    "5.4.15": "48005440f1a35d3d",  # Presumido
    # Versões 5.3.x - Hashes antigos (servidor diferente)
    "5.3.28": "3262e8f6b7bf8d66",
    "5.3.26": "3262e8f6b7bf8d66",
    "5.3.22": "3262e8f6b7bf8d66",
    "5.3.21": "1af9b7ee9c3886bd",
    "5.3.20": "3262e8f6b7bf8d66",
    # Versões 5.2.x - Hashes legados (servidor diferente)
...
def descobrir_url_por_scraping(versao, sid=None):
    ...
    MELHORADO: Agora suporta múltiplos padrões de URL e extrai hash corretamente.
    """
    def _log(msg):
    ...
    for url_blog in urls_tentadas:
        try:
            ...
            # MELHORADO: Padrão mais robusto que captura URLs com e sem hash
            padrao = r'https?://[^\s"\'<>]+(?:PEC/)?[a-zA-Z0-9_-]*/\d+\.\d+\.\d+/eSUS-AB-PEC-\d+\.\d+\.\d+-Linux64\.jar'
            ...
            if matches:
                ...
                # MELHORADO: Suporta URLs com e sem hash
                m = re.search(r'/PEC/([^/]+)/\d+\.\d+\.\d+/', url_jar)
                if m:
                    hash_encontrado = m.group(1)
                    # Valida se é um hash (não é um número de versão)
                    if not re.match(r'^\d+\.\d+\.\d+$', hash_encontrado):
                        _cache_hashes[versao] = hash_encontrado
                        _log(f"Hash extraído: {hash_encontrado}")
                
                return url_jar
            else:
                _log("Nenhum link .jar encontrado na página com padrão esperado.")
...
def descobrir_url_por_probe(versao, sid=None):
    ...
    MELHORADO: Testa múltiplos padrões de URL.
    """
    def _log(msg):
        if sid and sid in sessoes:
    ...
    # NOVO: Tenta padrão sem hash (alguns servidores podem ter reorganizado)
    _log("Tentando padrão sem hash...")
    url_sem_hash = f"{JAR_BASE}/{versao}/{nome_jar}"
    if _tentar_url(url_sem_hash):
        _log(f"✓ URL sem hash funciona!")
        return url_sem_hash
...
def buscar_url_versao(versao, url_manual=None, sid=None):
    ...
    Ordem de tentativas:
      ...
      6. NOVO: Tentativa com padrão sem hash
    
    MELHORADO: Melhor logging e tratamento de erros.
    """
    if url_manual:
        if sid and sid in sessoes:
            log(sid, f"  Usando URL manual: {url_manual}", "cmd")
        return url_manual

    # Cache em memória
    if versao in _cache_hashes:
        h = _cache_hashes[versao]
        url = f"{JAR_BASE}/{h}/{versao}/eSUS-AB-PEC-{versao}-Linux64.jar"
        if sid and sid in sessoes:
            log(sid, f"  Hash encontrado em cache: {h}", "cmd")
        return url
    ...
    if versao in HASHES_CONHECIDOS:
        ...
        url = f"{JAR_BASE}/{h}/{versao}/eSUS-AB-PEC-{versao}-Linux64.jar"
        if sid and sid in sessoes:
            log(sid, f"  Hash encontrado na lista estática: {h}", "cmd")
        return url
            ...
            if code != 0:
                ...
                if url_jar2 and url_jar2 != url_jar:
                    ...
                    if code == 0:
                        ...
                        falha(f"Falha no download mesmo após nova tentativa: {err}")
                        ssh_app.close()
                        return
                else:
                    falha(f"Falha no download: {err}")
                    ssh_app.close()
                    return
...
def api_ambientes():
    with get_db() as db:
        rows = db.execute("SELECT * FROM ambientes ORDER BY nome").fetchall()
    return jsonify([row_to_dict(r) for r in rows])

@app.route("/api/ambiente/<int:aid>", methods=["GET"])
...
def api_criar_ambiente():
    ...
    with get_db() as db:
        ...
        db.execute(
            ...
            (aid, d.get("host_app", "localhost"), d.get("porta_app", 22), d.get("usuario_app", "root"), 
             d.get("senha_app", ""), d.get("esus_dir", "/opt/e-SUS"), d.get("service", "e-SUS-PEC"), 0)
        )
        db.execute(
            "INSERT INTO srv_bd (ambiente_id, host, porta, usuario, senha, db_name, container) VALUES (?,?,?,?,?,?,?)",
            (aid, d.get("host_bd", "localhost"), d.get("porta_bd", 22), d.get("usuario_bd", "root"),
             d.get("senha_bd", ""), d.get("db_name", "esus"), d.get("container", "postgresql-db-1"))
        )
...
def api_resolver_url():
    ...
    if not versao:
        return jsonify({"erro": "Versão obrigatória"}), 400
    
    url = buscar_url_versao(versao, url_manual)
    if not url:
        return jsonify({"erro": f"URL não encontrada para versão {versao}"}), 404
...
def api_iniciar():
    ...
    sid = nova_sessao(modo_auto=False)
    threading.Thread(
